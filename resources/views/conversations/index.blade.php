            ...
            <div class="flex flex-col mb-4">
              <x-input-label for="selected_user" :value="__('会話したい相手（相互フォローユーザー）')" class="text-left mt-4" />
                ...
                @if(isset($selected_user))
                  <p class="mt-4 text-gray-800 dark:text-gray-200">{{ __('相手: ') . $selected_user->nickname }}</p>
                @endif
              </div>
            </div>
          </div>
...
<div class="sent text-right mb-4">
    ...
    <p class="text-xs text-gray-600" style="text-align: right; padding-left: 1rem; padding-right: 1rem;">{{ $conversation->created_at->format('Y-m-d H:i') }}</p>
</div>
@else
<div class="received text-left mb-4">
    <p class="inline-block bg-gray-200 rounded px-4 py-2">{{ $conversation->content }}</p>
    <p class="text-xs text-gray-600" style="text-align: left; padding-left: 1rem; padding-right: 1rem;">{{ $conversation->created_at->format('Y-m-d H:i') }}</p>
</div>
@endif
@endforeach
</div>
...
@else
<p class="text-gray-800 dark:text-gray-200">会話したい相手を選ぶと、これまでの会話が表示されます。</p>
@endif
</div>
</div>
</div>
